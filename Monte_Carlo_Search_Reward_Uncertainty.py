...
c = open('random_sample_grid_100_10.pickle', 'rb');
Train = pickle.load(c);
c.close()
LL=np.array(Train)
...
clusters = open('random_cluster_grid_100_10.pickle', 'rb');
...
pred = open('random_prediction_grid_100_10.pickle', 'rb');
...
def FindTraj(n,starting_pt,array_all=DLL1,Dll_xy=Dll_xy,New=New,clustering=clustering,XY_arr=XY_arr,dep_clus=dep_clus,M=M):
    ...
    for g in range(n-1):
        ...
        k1a = GPy.kern.RBF(input_dim=2,ARD=True, variance=m2.sum.rbf.variance[0], lengthscale=[m2.sum.rbf.lengthscale[0],m2.sum.rbf.lengthscale[1]])    
        k2a = GPy.kern.Matern32(input_dim=2,ARD=True, variance=m2.sum.Mat32.variance[0], lengthscale=[m2.sum.Mat32.lengthscale[0],m2.sum.Mat32.lengthscale[1]])
        k3a = GPy.kern.Matern52(input_dim=2,ARD=True, variance=m2.sum.Mat52.variance[0], lengthscale=[m2.sum.Mat52.lengthscale[0],m2.sum.Mat52.lengthscale[1]])
        ka=k3a+k2a+k1a 
        ...
        m3=GPy.core.GP(X=XY_arr_copy[clus_pred[0]], Y=dep_clus_copy[clus_pred[0]], kernel=ka,mean_function=mf1,likelihood=gauss, inference_method=exact)
        mean1,var1=m3.predict(XY_test1, full_cov=False, Y_metadata=None, kern=None, likelihood=None, include_likelihood=True)
        Total_Uncrt+=(lam**(g+2))*var1[0][0]
...
for s in range(49):
    ...
    for ic in in_circle:
        R=[]
        for r in range(5):
            trj=FindTraj(n=5000-s*100,starting_pt=ic[1][2])[0]
            R.append(trj)
        avg=sum(R)/len(R)
    ...
    for i in range(100):
        ...
        for ic1 in in_circle_l:
            R_l=[]
            for r3 in range(5):
                trj1=FindTraj(n=5000-s*100-r,starting_pt=ic1[1][2])[0]
                R_l.append(trj1)
                print(s,i,r3,trj1)
...
dm = open('gebco_indian_1_5000_100.pickle', 'wb')
pickle.dump( New , dm)   
dm.close()
